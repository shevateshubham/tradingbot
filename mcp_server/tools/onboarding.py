...
def build_market_keyboard(selected):
    ...
    for i,market in enumerate(MARKETS):
        mid=market["id"];is_sel=mid in selected;tick="✅" if is_sel else "☐"
        parts=market["label"].split();emoji=parts[0];rest=" ".join(parts[1:])
        full=f"{emoji} {tick} {rest}"
        mbtns.append(InlineKeyboardButton(full,callback_data=f"onboard_market_{mid}"))
...
def build_timeframe_keyboard(selected_tf):
    ...
    rows.append([InlineKeyboardButton("← Back",callback_data="onboard_step1"),InlineKeyboardButton("✅ Continue →",callback_data="onboard_step3") if selected_tf else InlineKeyboardButton("⬜ Select style first",callback_data="onboard_noop")])
    return InlineKeyboardMarkup(rows)

async def send_step2(update,context,session,edit=True):
    chat_id=str(update.effective_chat.id)
    user=await get_or_create_user(session,chat_id)
    tf=user.timeframe_mode or ""
    text=(
        "⏱ *What style of trading do you prefer?*\n\n"
        "This affects which timeframe signals I send you\\."
    )
...
def build_quality_keyboard(selected_qf):
    ...
    rows.append([InlineKeyboardButton("← Back",callback_data="onboard_step2"),InlineKeyboardButton("✅ Finish →",callback_data="onboard_step4") if selected_qf else InlineKeyboardButton("⬜ Select quality first",callback_data="onboard_noop")])
    return InlineKeyboardMarkup(rows)

async def send_step3(update,context,session,edit=True):
    chat_id=str(update.effective_chat.id)
    user=await get_or_create_user(session,chat_id)
    qf=user.quality_filter or ""
    text=(
...
async def handle_onboard_callback(update,context,session):
    ...
    if data=="onboard_noop":
        ...
        from mcp_server.config import QUALITY_FILTERS
        stmt=sql_update(UserPreferences).where(UserPreferences.chat_id==chat_id).values(markets_enabled=selected)
        ...
        from sqlalchemy import update as sql_update
        stmt=sql_update(UserPreferences).where(UserPreferences.chat_id==chat_id).values(timeframe_mode=tf)
        await session.execute(stmt);await session.commit()
        ...
        max_sigs=qf_data.get("max_signals",5)
        from sqlalchemy import update as sql_update
        stmt=sql_update(UserPreferences).where(UserPreferences.chat_id==chat_id).values(quality_filter=qf,max_signals_per_day=max_sigs)
        await session.execute(stmt);await session.commit()
