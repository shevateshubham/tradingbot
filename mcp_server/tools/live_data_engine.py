...
INDICES=[{"symbol":"NIFTY","exchange":"NSE","segment":"INDICES","lot_size":50},{"symbol":"BANKNIFTY","exchange":"NSE","segment":"INDICES","lot_size":15},{"symbol":"FINNIFTY","exchange":"NSE","segment":"INDICES","lot_size":40},{"symbol":"MIDCPNIFTY","exchange":"NSE","segment":"INDICES","lot_size":75}]
...
class LiveDataEngine:
    ...
    async def _process(self,sym,tf,inst_info):
        try:
            ...
            o=m["opens"];h=m["highs"];l=m["lows"];c=m["closes"];v=m["volumes"]
            cur=c[-1]
            def htf(cl):
        ...
        while self._running:
            try:
                if self.is_open(): await self._tick();await asyncio.sleep(60)
                    ...
                    mins=self.mins_to_open();sl=1800 if mins>60 else 60
                    logger.info(f"Market closed. Opens in {mins}min.");await asyncio.sleep(sl)
            except asyncio.CancelledError: break
            except Exception as e: logger.error(f"Engine: {e}",exc_info=True);await asyncio.sleep(30)
        self._running=False
